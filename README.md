...
> ⚠️ Experimental
> This UI DSL is under active development. Breaking changes may occur.
...
- `macros.window.ui.root([options,] ...children)`: creates the top-level container. Calling `.toHtml()` on the root returns the final HTML string.
  ...
  Options:
    - `errorRelay?: false`: relays errors in the WebView as a `macro:error` message (generated by default).
      This gives your WebView's scripts access to:
      - `macro.error(err)`: manually relay an error to the extension.

      The relay also automatically forwards:
...
#### Execution Context
Scripts run inside the WebView's JavaScript context, not the extension host.  
Your macro must enable script execution in the WebView by using `enableScripts: true` (templates do this by default). Refer to the [Scripts and message passing](https://code.visualstudio.com/api/extension-guides/webview#scripts-and-message-passing) documentation for details.
