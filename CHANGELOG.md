...
## 0.7.5
- UI DSL:
  - `macros.window.ui.tree` generates an `id` for nodes that do not define one.
  ...
  - Fix: `macros.window.handleLogMessage` logs unnecessary `null` value.
...
- Fix: Claimed Tree/WebView IDs are now auto‑released on macro completion. This is limited to non‑persistent and non‑retained macros.
