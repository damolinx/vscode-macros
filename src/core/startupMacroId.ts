...
export function getStartupMacroUri(uri: vscode.Uri) {
  return isStartup(uri) ? uri : uri.with({ scheme: 'startup', fragment: uri.scheme });
}

export function getMacroUriFromStartupMacroId(id: StartupMacroId) {
  const startupUri = vscode.Uri.parse(id);
  return getMacroUriFromStartupMacroUri(startupUri);
}

export function getMacroUriFromStartupMacroUri(uri: vscode.Uri) {
