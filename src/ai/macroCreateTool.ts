...
export function registerMacroCreateLanguageModelTool(context: ExtensionContext): vscode.Disposable {
  return vscode.lm.registerTool(CREATE_MACRO_TOOL_ID, new MacroCreateTool(context));
}
...
export class MacroCreateTool implements vscode.LanguageModelTool<MacroCreateToolArgs> {
  ...
  public async invoke(
    options: vscode.LanguageModelToolInvocationOptions<MacroCreateToolArgs>,
    ...
    return new vscode.LanguageModelToolResult([new vscode.LanguageModelTextPart(loadedTemplate)]);
  }
}
