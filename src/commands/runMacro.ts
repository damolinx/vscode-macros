...
export async function runMacro({ libraryManager, log, runnerManager, mruMacro }: ExtensionContext, pathOrUri?: PathLike, startup?: true) {
  const uri = pathOrUri
    ? toUri(pathOrUri)
  ...
  if (code.length === 0) {
    log.warn('Skipped running macro file: file is empty.', `"${vscode.workspace.asRelativePath(uri, true)}"`);
    return;
  }
